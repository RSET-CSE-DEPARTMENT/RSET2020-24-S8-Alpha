...
# from fcon import user_id as uid
import sqlite3
...
np.random.seed(42)
df = pd.read_csv('C:/Users/LAKSHMI/Downloads/SecureVoteX/SecureVoteX/train.csv')
# Drop pose_id column
...
existing_root = None
class VideoPlayer:
    def _init_(self, master, video_source):
        self.master = master
    ...
    def display_video(self):
        ...
        if prediction[0]=='sit ups':
                    ...
                    WIDTH = 10000
                    HEIGHT = 10000
                    self.cap.set(cv2.self.cap_PROP_FRAME_WIDTH, WIDTH)
                    self.cap.set(cv2.self.cap_PROP_FRAME_HEIGHT, HEIGHT)
                    width = int(self.cap.get(cv2.self.cap_PROP_FRAME_WIDTH))
                    height = int(self.cap.get(cv2.self.cap_PROP_FRAME_HEIGHT))
        ...
        if start_time is not None and end_time is not None:
                        workout_duration = end_time - start_time
                        print("Workout duration:", workout_duration, "seconds")    
                        work_duration = workout_duration
        counter = count
        c.execute("INSERT INTO report (user_id, exercise, count, work_duration) VALUES (?, ?, ?, ?)",
          (user_id, exercise, count, work_duration))
